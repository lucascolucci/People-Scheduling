...
for (i in 0:(number_of_people-1)) {
  for (j in 1:number_of_projects) {
    one_project_per_person_constraint[j+i*number_of_projects+i*number_of_people*number_of_projects] = 1 
  }
}
...
for (i in 0:(number_of_projects-1)) {
  for(j in 0:(number_of_people-1)){
    senior_constraint[(1+(number_of_projects-1)*j)+j+i+i*number_of_people*number_of_projects] = qi[j+1]
  }
}
...
for (i in 0:(number_of_projects-1)) {
  for(j in 0:(number_of_people-1)){
    number_of_people_per_project_contraint[(1+(number_of_projects-1)*j)+j+i+i*number_of_people*number_of_projects] = 1
  }
}
...
f.con <- matrix(
                ...
                nrow=(number_of_people+number_of_projects+number_of_projects), byrow=TRUE)
